...
const LocalAlertContext = createContext<LocalAlertContextType | undefined>(
  undefined,
);

export const LocalAlertProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const [localAlert, setLocalAlert] = useState({ visible: false, message: "" });
  const [timeoutId, setTimeoutId] = useState<NodeJS.Timeout | null>(null);

  const showLocalAlert = useCallback(
  ...
  const hideLocalAlert = useCallback(() => {
    setLocalAlert({ visible: false, message: "" });
    if (timeoutId) {
      clearTimeout(timeoutId);
      setTimeoutId(null);
  ...
  return (
    <LocalAlertContext.Provider
      value={{ showLocalAlert, hideLocalAlert, localAlert }}
    >
      {children}
