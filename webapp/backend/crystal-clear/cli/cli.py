...
def dependency(
    ...
    try:
        cc = CrystalClear(url=node_url, allium_api_key=allium_key)
...
def code(etherscan_api_key, address, log_level):
    ...
    try:
        cc = CrystalClear(url=None, etherscan_api_key=etherscan_key)

        proxy_info: ProxyInfo = cc.get_proxy_info(address)
        permissions_info: PermissionsInfo = cc.get_permissions_info(address)

        # --- Metadata Panel ---
        metadata = (
            f"[bold]Contract:[/bold] {address}\n"
            f"[bold]Proxy Information:[/bold] {proxy_info.description if proxy_info else 'Not a Proxy'}\n"
            f"[bold]Permissioned functions:[/bold] {len(permissions_info.permissions) if permissions_info else 0}"
        )
        ...
        if not permissions_info or len(permissions_info.permissions) == 0:
            console.print("[green]No permissioned functions detected.[/green]")
            return

        # --- Functions Table ---
        func_table = Table(title="⚠️ Permissioned Functions", header_style="bold red")
        ...
        for item in permissions_info.permissions:
