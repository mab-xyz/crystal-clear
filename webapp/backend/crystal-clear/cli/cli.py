...
@click.option(
    "--allium-api-key", type=str, help="Allium API key",
)
...
@click.option("--scope", default="single", type=click.Choice(["single", "supply-chain"]), help="Analysis scope")
@click.option("--address", required=True, type=str, help="Contract address")
@click.option("--from-block", required=False, type=str, help="Starting block number (for supply-chain scope)")
@click.option("--to-block", required=False, type=str, help="Ending block number (for supply-chain scope)")
@click.option("--blocks", default=5, type=int, help="Number of blocks to fetch at a time (for supply-chain scope)")
@click.option("--log-level", default="ERROR", type=str, help="Logging level")
@click.option("--export-json", type=str, help="Export call graph to JSON file")
def risk(etherscan_api_key, node_url, scope, address, from_block, to_block, blocks, log_level, export_json):
    ...
    try:
        ...
        metadata_general = (
            ...
            f"[bold]All Verified Contracts:[/bold] {"✅" if analysis.aggregated_risks.verified else "❌"}\n"
            f"[bold]Aggregated Risks:[/bold] {analysis.aggregated_risks.risk_factors}"
        )

        table = Table(title=f"📊 Dependency Risk Summary", header_style="bold red", expand=True, show_lines=True)
        ...
        for i, dep in enumerate(sorted_deps):
            if i >= 5:
                table.add_row("...", "...", f"... {len(analysis.dependencies) - 5} more ...")
                break
            table.add_row(
                dep.address,
        ...
        console.print(
            Panel(
                group_general,
                title=f"Aggregated Risk Analysis",
        ...
        for i, dep in enumerate(sorted_deps):
            if i >= 3:
                console.print(f"[yellow]... ({len(analysis.dependencies) - 3} more contracts omitted)[/yellow]")
                break

            details = dep.details or {}
            verification_data = details.get("verification_info") or None
            verification_info = None if not verification_data else VerificationDetails(**verification_data)
            ...
            permissions_info = None if not permissions_data else PermissionsInfo(**permissions_data)
