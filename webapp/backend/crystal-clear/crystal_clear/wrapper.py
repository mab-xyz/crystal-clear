...
from crystal_clear.code_analyzer import Analyzer, ProxyInfo, PermissionsInfo, Risk, RiskFactors
...
class RiskAnalysis(BaseModel):
    ...
    dependencies: List[DependencyRisk] = Field(..., description="List of analyzed dependencies with risk factors")
    aggregated_risks: Risk = Field(..., description="Aggregated risk factors across all dependencies")
...
class CrystalClear:
    ...
    def get_risk_factors(self, address: str, scope: str, from_block: str = None, to_block: str = None, blocks: int = 5) -> RiskAnalysis:
        if not scope in ["single", "supply-chain"]:
        ...
        if scope == "single":
            ...
            if not self.trace_collector:
                raise ValueError("TraceCollector is not initialized. Please provide a node url.")
            callgraph: CallGraph = self.get_dependencies(address, from_block, to_block, blocks=blocks)
            aggregated = Risk(
                verified=True,
            ...
            analysis = RiskAnalysis(root_address=address, 
            ...
            for addr in callgraph.nodes.keys():
                try:
                    ...
                    dependency_risk = DependencyRisk(
                        address=addr,
                        dependency_depth=callgraph.dependency_depths.get(addr.lower(), 0),
                        **risk.model_dump()
                    )
