...
from api.core.exceptions import InternalServerError, NotFoundError, InputValidationError
...
def get_verification_data(address: str) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        response: VerificationDetails = cc.sourcify_client.check_contract_verified(address.lower())
        if response.verification == "not-verified":
            logger.info(f"Sourcify: Contract {address} not verified. Checking Etherscan...")
