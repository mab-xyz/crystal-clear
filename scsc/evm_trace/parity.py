...
def get_unique_addresses_from_parity_trace(
    traces: ParityTraceList,
    root: Optional[ParityTrace] = None,
    **root_kwargs,
) -> set[str]:
    """
    Create a set of unique addresses from output models using the Parity approach
    (e.g., from the ``trace_transaction`` RPC).
    ...
    Args:
        ...
        root (:class:`~evm_trace.parity.ParityTrace`): The root parity trace node. Optional, uses
          the first item by default.
        **root_kwargs: Additional kwargs, if needed for future use.

    Returns:
        set[str]: A set of unique addresses involved in the transaction.
    """
    root = root or traces.root[0]
    ...
    if root.call_type == CallType.CREATE:
        # create_action: CreateAction = cast(CreateAction, root.action)
        # # Add the address that initiated the CREATE action
        # unique_addresses.add(create_action.from_address)
        pass
        ...
        call_action: CallAction = cast(CallAction, root.action)
        # Add both the sender and the receiver addresses
        unique_addresses.add(call_action.receiver.lower())

    elif root.call_type == CallType.SELFDESTRUCT:
        # selfdestruct_action: SelfDestructAction = cast(SelfDestructAction, root.action)
        # # Add the address related to SELFDESTRUCT
        # unique_addresses.add(selfdestruct_action.address)
        pass

    # Recursively handle subtraces
    trace_list: list[ParityTrace] = traces.root
    subtraces: list[ParityTrace] = [
        sub
        for sub in trace_list
    ...
    for sub in subtraces:
        unique_addresses.update(get_unique_addresses_from_parity_trace(traces, root=sub))
